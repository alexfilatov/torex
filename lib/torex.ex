defmodule Torex do
  ...
  def start(_type, _args) do
    ...
    children = [
      :hackney_pool.child_spec(:torex_pool, [timeout: 60_000, recv_timeout: 60_000, max_connections: 1_000])
    ]
  ...
  defp request(method, url, body \\ [], headers \\ []) when method == :get or method == :post do
    case HTTPClient.request(method, url, body, headers,
      hackney: [:insecure] ++ [pool: :torex_pool, proxy: {:socks5, @tor_server[:ip], @tor_server[:port]}]) do
